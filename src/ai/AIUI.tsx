  ...
  const conversationArr = [
    {
      role: "system",
      content:
        "You are an planning expert giving short answers. Your text answer must not pass the limit of 100 words. Every question should be broken into tasks and if needed into task's subtasks. For example, if there is a question about planning vacation, the answer should explain what are the tasks and microtasks for vacation. Please provide no more than 3 subtasks for every task. Also if needed to provide subtask for a subtask, do not exceed 3 subtasks for a subtask.",
    },
  ];
  const renderedArr = [
    { role: "assistant", content: "What are you planning?" },
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "https://todolist-with-integrated-ai-chat.netlify.app/.netlify/functions/fetchAI",
        {
          method: "POST",
          headers: {
